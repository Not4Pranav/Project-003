...
import { EmixFGError, ErrorTypes, replyUserError } from '../../utils/errorHandler.js';
    ...
    async execute(interaction, config, client) {
        try {
            ...
            try {
                ...
                if (error instanceof EmixFGError) {
                    errorMessage = error.userMessage || 'An error occurred. Please try again.';
                    logger.debug(`EmixFGError [${error.type}]: ${error.message}`, error.context || {});
                } else {
    ...
    try {
        ...
        collector.on('collect', async (buttonInteraction) => {
            try {
                ...
                const userMessage = error instanceof EmixFGError
