...
**EmixFG** is a powerful, feature-rich Discord bot designed to enhance your server experience with comprehensive moderation tools, engaging economy systems, utility features, and much more. Built with modern Discord.js v14 and PostgreSQL for optimal performance and data persistence.
...
EmixFG offers a complete suite of tools for Discord server management and community engagement:
...
[**EmixFG Setup Tutorial**](https://www.youtube.com/@TouchDisc)

## Docker Deployment (Recommended)

EmixFG is fully containerized for easy deployment.
...
2. **Configure environment variables:**
   ...
   cp .env.example .env
   ```
   Set at minimum `DISCORD_TOKEN`, `CLIENT_ID`, and `GUILD_ID`. Docker Compose also reads `POSTGRES_USER`, `POSTGRES_PASSWORD`, and `POSTGRES_DB` from `.env` (defaults: `emixfg` / `password` / `emixfg`).
...
Music uses [Lavalink v4](https://github.com/lavalink-devs/Lavalink) via [Riffy](https://github.com/riffy-rb/riffy), similar to [Musicify](https://github.com/Not4Pranav/Musicify).
...
```bash
docker pull ghcr.io/Not4Pranav/emixfg:main
```
...
3. **Configure Environment Variables**
   ...
   POSTGRES_URL=postgresql://postgres:yourpassword@localhost:5432/emixfg
   ...
   - If your chosen `PORT` is already used, EmixFG automatically tries the next port(s)
...
4. **Setup PostgreSQL Database** (Optional but recommended)
   ...
   createdb emixfg
   createuser emixfg
   psql -c "ALTER USER emixfg PASSWORD 'yourpassword';"
   psql -c "GRANT ALL PRIVILEGES ON DATABASE emixfg TO emixfg;"
   ```
...
EmixFG requires the following Discord intents:
- **Guilds**
- **Guild Messages**
- **Message Content**
- **Guild Members**
